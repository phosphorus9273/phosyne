...
(async function (util, global, settings, Canvas, color, gameDraw, socketStuff) {
  ...
  window.onload = async () => {
    const servers = [
        ...
        /*["lopsided-puzzling-shell.glitch.me", "KDTREE", true, 0], // KE TREE
        ["goldenrod-imported-iguana.glitch.me", "RTREE", true, 0], // R TREE
  ...
  function drawMinimapAndDebug(spacing, alcoveSize, GRAPHDATA) {
    ...
    ctx.lineWidth = 2;
    ctx.fillStyle = color.dgrey;
    drawGuiRect(x, y, len+2, height+2, true); // Border
    ...
    // Text
    if (global.showDebug) {
        drawText("Open Source Arras", x + len, y - 50 - 5 * 14 - 2, 15, "#ffffff", "right");
